...
var Form = function () {
    ...
    _createClass(Form, [{
        ...
        value: function setupData(data) {
            ...
            var stringed = this.stringValues(data);
        ...
        key: 'stringValues',
        value: function stringValues(data) {
            var _this2 = this;

            if (data instanceof Array) {
                return data.map(function (el) {
                    return _this2.stringValues(el);
                });
            } else if (data instanceof Object) {
                var obj = {};
                Object.getOwnPropertyNames(data).filter(function (elId) {
                    return elId.indexOf('__') !== 0;
                }).forEach(function (elId) {
                    return obj[elId] = _this2.stringValues(data[elId]);
                });
                ...
                return new Promise(function (resolve, reject) {
                    return promise.then(function (data) {
                        ...
                        if (error.errors != null) {
                            ...
                            _this4.errors.clear();
                            _this4.errors.record(error.errors);
                            _this4.$v.$touch();
                            _this4.$bus.$emit('validate');
                            // reject(error);
                        }
                    });
                });
                    ...
                    if (this.hasValidator(event.field)) {
                        ...
                        if (this._config.validateOnInput) {
                            ...
                            this._timers.inputDebounce = setTimeout(function () {
                                return _this5.validate(event.field);
                        ...
                        if (validation.hasOwnProperty(rule) && validation[rule] === false) {
                            ...
                            for (var param in params) {
                                if (params.hasOwnProperty(param)) {
                                    var replaceWith = JSON.parse(JSON.stringify(params[param]));
                                    if (param === 'array') {
                                        var lastEl = replaceWith[replaceWith.length - 1];
